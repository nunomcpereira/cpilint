...
public final class ZipArchiveIflowArtifact implements IflowArtifact {
	...
	public Collection<ArtifactResource> getResourcesByType(ArtifactResourceType type) {
		...
		if (!resources.containsKey(type)) {
			throw new IflowArtifactError("Artifact [" + type + "] resource type not found");
		}
	...
	private static void replaceExternalParameters(Map<String, byte[]> contents) throws IOException, SaxonApiException {
		...
		byte[] originalBytes = contents.get(iflowXmlPath);
		byte[] copiedArray = new byte[originalBytes.length];
		System.arraycopy(originalBytes, 0, copiedArray, 0, originalBytes.length);
		contents.put(iflowXmlPath.replace(".iflw","_ORIGINAL.iflw"),copiedArray);
		InputStream iflowXml = new ByteArrayInputStream(originalBytes);
